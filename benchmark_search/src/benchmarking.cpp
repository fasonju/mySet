...
int main(int argc, char *argv[]) {
    ...
    if (modeStr == "avl") {
        if (variation == "random") {
            if (datasetStr == "tiny") {
                duration = benchmarkSearch<AVLTree<int>>(TINY_UNIFORM);
            }

            if (datasetStr == "medium") {
                duration = benchmarkSearch<AVLTree<int>>(MEDIUM_UNIFORM);
            }

            if (datasetStr == "large") {
                duration = benchmarkSearch<AVLTree<int>>(LARGE_UNIFORM);
            }
        } else if (variation == "sorted") {
            if (datasetStr == "tiny") {
                duration = benchmarkSearch<AVLTree<int>>(TINY_SORTED);
            }

            if (datasetStr == "medium") {
                duration = benchmarkSearch<AVLTree<int>>(MEDIUM_SORTED);
            }

            if (datasetStr == "large") {
                duration = benchmarkSearch<AVLTree<int>>(LARGE_SORTED);
            }
        } else {
            if (datasetStr == "tiny") {
                duration = benchmarkSearch<AVLTree<int>>(TINY_REVERSE);
            }

            if (datasetStr == "medium") {
                duration = benchmarkSearch<AVLTree<int>>(MEDIUM_REVERSE);
            }

            if (datasetStr == "large") {
                duration = benchmarkSearch<AVLTree<int>>(LARGE_REVERSE);
            }
        }
    } else {
        if (variation == "random") {
            if (datasetStr == "tiny") {
                duration = benchmarkSearchSet<>(TINY_UNIFORM);
            }

            if (datasetStr == "medium") {
                duration = benchmarkSearchSet<>(MEDIUM_UNIFORM);
            }

            if (datasetStr == "large") {
                duration = benchmarkSearchSet<>(LARGE_UNIFORM);
            }
        } else if (variation == "sorted") {
            if (datasetStr == "tiny") {
                duration = benchmarkSearchSet<>(TINY_SORTED);
            }

            if (datasetStr == "medium") {
                duration = benchmarkSearchSet<>(MEDIUM_SORTED);
            }

            if (datasetStr == "large") {
                duration = benchmarkSearchSet<>(LARGE_SORTED);
            }
        } else {
            if (datasetStr == "tiny") {
                duration = benchmarkSearchSet<>(TINY_REVERSE);
            }

            if (datasetStr == "medium") {
                duration = benchmarkSearchSet<>(MEDIUM_REVERSE);
            }

            if (datasetStr == "large") {
                duration = benchmarkSearchSet<>(LARGE_REVERSE);
            }
        }
    }
    std::cout << duration << "\n";
